...
foreach ($allMessages as $resultmes) {
    ...
    if (!in_array($threadnumber, $threadcount)) {
        ...
        . "</span><br><span class='message_text_preview "
        .($current_thread == $resultmes['thread'] ? ' message_active_thread' : "message_inactive_thread")."'>" . $system->trimStr($resultmes['message'], 40) . "</span></div>"
...
if ($messagecount == 0) {
    echo "<li class='inbox_message' style='padding:20px; text-align: center;'>No messages</li> ";
}
?>
                    </ul>
                </div>
