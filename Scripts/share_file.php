<?php
	...
	foreach($receivers as $receiver)
	{
		...
		if($split[0] == "school")
		{
			$sql = "INSERT INTO file_share(user_id, file_id, community_id) VALUES (:user_id, :file_id, :community_id);";
			$sql = $database_connection->prepare($sql);
			$sql->execute(array(":user_id" => $user->getId(), ":file_id" => $file_id, ":community_id" => $split[1]));
			if(!$database_connection->query("INSERT INTO activity_share(activity_id, community_id)
			 VALUES(".$activity_id.", ".$split[1].");"))
			{
				echo "error_share/";
			}
		...
		if($split[0] == "year")
		{
				$sql = "INSERT INTO file_share(user_id, file_id, community_id, year) VALUES (:user_id, :file_id, :community_id, :year);";
			$sql = $database_connection->prepare($sql);
			$sql->execute(array(":user_id" => $user->getId(), ":file_id" => $file_id, ":community_id" => $split[1], ":year" => $split[2]));
