...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($count == 1) {
        ...
        $user_query = "INSERT INTO users (name, password, community_id, school, year, email,  gender, first_name, last_name) "
                . "VALUES (:name, :password, :community_id, :school, :year, :email, :gender, :first_name, :last_name);";
        $user_query = $database_connection->prepare($user_query, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
        $user_query->execute(
                array(
                    ":name" => $newname,
        ...
        $user_query = "SELECT id, name, position FROM users WHERE name = :name and school = :school LIMIT 1";
        $user_query = $database_connection->prepare($user_query, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
